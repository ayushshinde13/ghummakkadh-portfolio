    ...
    <Section className="bg-[#4eb902] py-16 border-y-4 border-[#FF7700]">
      <Container className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
        {stats.map((stat) => (
          <div
            key={stat.label}
            className="flex flex-col gap-1 bg-white/90 backdrop-blur-sm p-6 rounded-3xl border-2 border-[#FF7700] shadow-sm hover:scale-105 transition-transform"
          >
            <Heading level={2} className="text-[#CC5F00]">
              {stat.value}
            </Heading>
