    ...
    <header className="fixed top-0 left-0 right-0 w-full z-50 bg-[#0A0E1A] border-b border-white/10 shadow-[0_4px_30px_rgba(0,0,0,0.3)] transition-all duration-300">
      <Container className="flex items-center justify-between h-20">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <button
            className="xl:hidden p-2 text-gray-200 hover:text-white transition-colors bg-white/10 rounded-full"
      ...
      {isMobileMenuOpen && (
        <div className="xl:hidden absolute top-[90px] left-0 w-full bg-[#0A0E1A]/95 backdrop-blur-xl border border-white/10 shadow-[0_10px_40px_rgba(0,0,0,0.3)] rounded-3xl py-4 px-4 flex flex-col gap-2 z-40 animate-in slide-in-from-top-2">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
